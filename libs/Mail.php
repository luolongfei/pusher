...
class Mail
{
    ...
    public function send($subject, $content, $to = '', $template = '')
    {
        ...
        // 邮件服务配置
        $mail->SMTPDebug = config('mail.debug'); // debug，正式环境应关闭 0：关闭 1：客户端信息 2：客户端和服务端信息
        ...
        $mail->setFrom(config('mail.from'), 'im robot'); // 发件人
        $mail->addAddress($to ?: config('mail.to'), '罗叔叔'); // 添加收件人，参数2选填
        $mail->addReplyTo(config('mail.replyTo'), '罗飞飞'); // 备用回复地址，收到的回复的邮件将被发到此地址
...
//        $mail->addAttachment('README.md', '说明.txt');
        ...
        $mail->msgHTML($message, APP_PATH . 'mail');
